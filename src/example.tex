...
\section{Introduction}
    ...
    The current development process for using the tool is to first manually download it (if not using the RIT CS servers which has it pre-installed), then to write a .plcc file containing the grammar rules, regexes for tokens, and extra java files for custom functionality, then run a command to generate the language interpreter, then run one of three REPL programs.
    ...
    Intellij also has the idea of a run configuration that is a graphical setup of what can essentially be boiled down to a command-line command.
