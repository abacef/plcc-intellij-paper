...
\begin{abstract}
    High level Integrated Development Environments aide with programming in many ways, especially when documentation for the language is not informative enough.
    This paper presents an Intellij platform plugin for an compiler compiler language used for teaching programming language concepts.
    It includes syntax highlighting and error detection, one click compiler download, simple run configurations, and easy new file creation.
\end{abstract}
...
A file called \textit{Scan.java} is generated which takes in a string of text (a program) and outputs a next token function which gives the next token found in the program string.
...
The output of feeding a program into the REPL is the string representation of the program, which consists of toString being called on each grammar element in recursive decent fashion.
